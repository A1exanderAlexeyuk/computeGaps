...
#'
#' @export
validate_tsv_structure <- function(tsv_data) {
  ...
  extra_cols <- setdiff(names(tsv_data), required_cols)
  
  # Check for valid JSON in predicate_metadata
  json_errors <- c()
  for (i in 1:nrow(tsv_data)) {
    tryCatch({
      jsonlite::fromJSON(tsv_data$predicate_metadata[i])
    }, error = function(e) {
      json_errors <<- c(json_errors, i)
    })
  }
